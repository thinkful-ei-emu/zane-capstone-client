...
export default function InventoryTable(props) {
  return (
    ...
    <button className='sortbutton' onClick={props.sortLocation}>{sort}Location</button> */}
        <Link className="createbutton" to="/create-form">
          <button className="createbutton sortbutton">Create Item</button>
        </Link>
      </div>
      <table>
        <tbody>
          <tr>
            <th
              onClick={
                props.state.sorted === false ? props.sortNameD : props.sortNameA
              }
            >
              {props.state.nameup === true ? (
                <p className="arrow">&uarr;</p>
              ) : (
                ""
              )}
              Item Name
              {props.state.nameup === false ? (
            ...
            <th
              ...
              onClick={
                props.state.sorted === false
                  ? props.sortPriceA
                  : props.sortPriceD
              }
            >
              <p onClick={props.sortPriceA}></p>
              {props.state.priceup === true ? (
                <p className="arrow">&uarr;</p>
              ) : (
                ""
              )}
              Unit Price
              {props.state.priceup === false ? (
                <p className="arrow">&darr;</p>
              ) : (
                ""
              )}
